...
public class HashMap<Key, Value> : IEnumerable<Pair<Key, Value>>
{
    ...
    public bool PutIfAbsent(Key key, Value value)
    {
        ...
        int shortcut = HashCode(key);

        if (_values[shortcut] == null)
        {
            _values[shortcut] = new List<Pair<Key, Value>>();
            _values[shortcut].Add(new Pair<Key, Value>() { Key = key, Value = value });
        }
        else
        {
            _values[shortcut].Add(new Pair<Key, Value>() { Key = key, Value = value });
    ...
    public bool Remove(Key key)
    {
        ...
        if (_values[shortcut].Find(i => i.Key.Equals(key)) != null)
        {
            int value = _values[shortcut].FindIndex(i => i.Key.Equals(key));

            _values[shortcut].RemoveAt(value);
            _keys.Remove(key);
            return true;
